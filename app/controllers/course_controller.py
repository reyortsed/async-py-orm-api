...
async def create_course(course_in: CourseCreate, service: CourseService = Depends(get_course_service)):
...
async def update_course(course_id: int, course_in: CourseUpdate, service: CourseService = Depends(get_course_service)):
...
async def add_user_to_course(course_id: int, user_id: int, service: CourseService = Depends(get_course_service)):
    return await service.add_user_to_course(course_id, user_id)

@router.get("/{course_id}/users", response_model=CourseWithUsers)
async def list_users_for_course(course_id: int, service: CourseService = Depends(get_course_service)):
...
async def remove_user_from_course(course_id: int, user_id: int, service: CourseService = Depends(get_course_service)):
