...
async def add_course_to_user(user_id: int, course_id: int, service: UserService = Depends(get_user_service)):
    return await service.add_course_to_user(user_id, course_id)

@router.get("/{user_id}/courses", response_model=UserWithCourses)
async def list_courses_for_user(user_id: int, service: UserService = Depends(get_user_service)):
...
async def remove_course_from_user(user_id: int, course_id: int, service: UserService = Depends(get_user_service)):
